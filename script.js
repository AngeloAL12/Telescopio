const canvas = document.getElementById("canvas");
        ...
        function drawImage() {
            ...
            const scaleFactor = Math.max(canvas.width / img.width, canvas.height / img.height) * (magnification / 100);
            const width = img.width * scaleFactor;
            const height = img.height * scaleFactor;
    
            // Dibujar la imagen sin fondo y permitir el movimiento libre
            ctx.drawImage(img, (canvas.width - width) / 2 + offsetX, (canvas.height - height) / 2 + offsetY, width, height);
